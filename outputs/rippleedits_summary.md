...
| criterion | type | Base | ROME | RAG | INLAY |
|---|---|---|---|---|---|
...
**Ranking: RAG 0.393 > INLAY 0.221 > ROME 0.078 > Base 0.073.**

## What this says — and it reverses the leaderboard

On the single-edit metrics (CounterFact/zsRE), INLAY was #1. **On native portability, INLAY is not — and
...
2. **ROME scores near base (0.078 vs 0.073) — indicative, not a rigorously matched comparison.** A
   ...
   guaranteed identical* to the base/RAG/INLAY sample, and the heuristic may extract wrong subject spans for
   edits it keeps. ROME's low number is therefore suggestive of the RippleEdits paper's point (weight-editing
   is no portability panacea) but should not be read as a rigorously matched result. The **RAG-vs-INLAY
   contrast is sound** — those three methods share an identical query set (no skip logic).
3. **INLAY shows exactly its predicted signature.** It *wins Subject_Aliasing* (0.816 — an alias of the
...
- **Single-edit efficacy / generalization / locality / sequential retention** (CounterFact, zsRE,
  sequential stress test): **INLAY is state of the art** — it leads base, RAG, fine-tune, ROME, MEMIT,
  WISE, GRACE, and AlphaEdit at two model sizes, sample-stable to N=1000, gradient-free, ~1600× cheaper
  per edit than ROME.
- **Compositional portability** (native RippleEdits): **INLAY is not SOTA, and cannot be by design.** RAG
  wins; INLAY places second overall but is *last* on preservation because it retrieves rather than
  re-derives. Even ROME can't win this axis.

So the precise, defensible claim is: **INLAY is state of the art for direct fact editing and lifelong/
...
(direct editing vs. ripple propagation) reward opposite mechanisms. INLAY optimizes the first; RAG the
second. That is the honest, complete answer, now backed by the field's own portability benchmark.

## Files
- `rippleedits.{json,png}`, `eval_rippleedits.py` (base/RAG/INLAY), `eval_rome_ripple.py` (ROME via EasyEdit)
