# INLAY: Gradient-Free Knowledge Editing via Semantically-Addressed Memory
...
slow, and they degrade the model as edits accumulate. We propose **INLAY (gradient-free, externally addressable knowledge
...
inference the slot is retrieved by a semantic key and played back in logit space. INLAY edits in
...
|---|---|---|---|---|---|
| **INLAY (ours)** | **1.00** | **0.87** | 0.84 | **0.90** | ~5 ms/edit |
...
*Sample sizes: INLAY N=1000, ROME N=2000 (matched-scale rerun), WISE/AlphaEdit N=100, base/RAG N=1000. ROME and INLAY are the two strongest; INLAY leads by ~0.10 at matched scale.*

- **INLAY's CounterFact score is flat across three model families** (harmonic-mean ES/PS/NS):
...
- **zsRE — INLAY is best on both large models:** GPT-J-6B **1.00** and Qwen2.5-7B **0.996**
  (vs RAG 0.88/0.87, vs ROME 0.91 on GPT-2-XL). Near-perfect efficacy, paraphrase, and locality.
- **Sequential editing (GPT-2-XL):** INLAY retains ~99.5% of 400 edits; ROME destroys the model by
...
- **Portability (native RippleEdits, matched protocol, both models, honest result):** RAG wins
  (0.40/0.44 aggregate); INLAY (0.23/0.29) beats the weight-editors on propagation but over-writes
...
2. **Multiple model families** — not just GPT-J. We are adding **Qwen2.5-7B** (done downloading; INLAY
...
|---|---|
| Gradient-free methods (INLAY/base/RAG), full-N, ×3 models, 2 benchmarks | ~45 h |
