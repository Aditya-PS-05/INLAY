# Multi-token value fix — INLAY v3

The zsRE benchmark showed INLAY v2's one remaining limit was its **value representation**, not its
...
INLAY does **position-wise logit-space playback** of the full stored answer sequence: at scored
...
## Full zsRE comparison — INLAY v3 now leads
...
| **INLAY v3** | **0.96** | **0.96** | **1.00** | **0.973** | **0.3 s** | **0** |

INLAY v3 is now the top method on zsRE — above ROME (0.908) and RAG (0.887) — while remaining
...
## Cross-benchmark summary (INLAY v3, held-out)
...
INLAY v3 leads ROME on **both** standard single-edit benchmarks, gradient-free and non-destructive
...
- Playback biases toward the stored answer tokens; it is retrieval of written content, appropriate for
  a memory method, but it means INLAY reproduces the answer *as written* rather than re-deriving it.
...
- `zsre.json` / `zsre.csv` — updated with INLAY v3
