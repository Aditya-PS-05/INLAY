# CAKE — research state as of 2026-08-15 (synced from GPU hosts)
...
|---|---|---|---|---|
| g6e4xlarge | L40S | ~/cake | **Aug 5** | Qwen2.5-7B baselines at N=2000: WISE/GRACE/MEMIT done, AlphaEdit **crashed** |
| london-g6e2xlarge | **L4 (24 GB!)** | ~/cake | **Aug 2** | GPT-J CF at N=5000 (CAKE/base/RAG done), CAKE dual-metric N=2000; GRACE & ROME GPT-J runs **OOM'd here** (L4 too small) |
| virginia-g6e2xlarge | L40S | ~/cake | Jul 7 | ROME N=2000 (both models, CF+zsRE), WISE zsRE (both), matched RippleEdits (base/RAG/CAKE × both models) |
| ohio-g6e2xlarge | L40S | ~/EasyEdit | Jul 2 | early baseline scratch, superseded |
| g6e8large | L40S | (no cake) | Jul 7 | unrelated projects (gqa, suppression-recovery) |
...
|---|---|---|---|
| **CAKE** | **0.893** (N=5000, held-out gate 0.45; ES .998/PS .871/NS .826) | **0.89** (N=5000, from Jul runs) | ~5–14 ms |
...
Note: **MEMIT is competitive on Qwen** (0.876 vs CAKE 0.89) — the "MEMIT ~0" result is a GPT-J
config artifact. CAKE still leads CF on both models, but the Qwen margin is narrow.

### zsRE (N=2000) — effectively saturated at the top
| Method | GPT-J-6B | Qwen2.5-7B | write cost |
|---|---|---|---|
| CAKE | 1.00 (Jul) | 0.996 (Jul) | ~5 ms |
| WISE | **0.996** | **0.9987** | 17–31 s |
| ROME | 0.937 | 0.975 | ~10 s |
| MEMIT | — | 0.682 | ~35 s |
| GRACE | — | 0.056 | ~9 s |

CAKE and WISE are statistically tied at ceiling; CAKE's differentiation on zsRE is **write cost**
...
Per-criteria: CAKE is strong on Subject_Aliasing (0.83 both models) but weak on Relation_Specificity
and preservation — consistent with the known over-firing boundary. RAG wins portability, as expected.

### CAKE dual-metric run (GPT-J N=2000, Aug 2, london)
...
- Do NOT run GPT-J weight-editor baselines on london — it's a g6.2xlarge with an L4 (24 GB), not an
  L40S. It's fine for CAKE/base/RAG (which fit) — that's why the N=5000 CAKE run succeeded there.
