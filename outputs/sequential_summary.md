# Sequential / batch editing stress test — CAKE's structural home turf
...
every edit, so interference accumulates; CAKE writes each fact to a **separate memory slot** with zero
...
|---|---|---|---|
| **CAKE v3** | **0.995 @ N=400** | 0.55 @ N=400 | **400 edits in 1.8 s, 0 grad** |
| ROME (sequential) | 0.00 (collapsed) | **0.00 @ N=50** | 100 edits in 758 s |
| In-context (RAG) | 0.94 @ N=400 | 1.00 | re-supplied per query |
| Base | ~0.00 | 1.00 | — |

**CAKE retains essentially all 400 edits (0.995); ROME destroys the model by ~50 edits.** This is the
clearest separation in the whole project. Per-edit write cost: CAKE 4.6 ms vs ROME 7.6 s (~1600×).

## What each curve shows

- **CAKE — retention is flat at 1.0 across three orders of magnitude of N** (falling only to 0.995 at
...
## CAKE's honest cost: locality degrades with memory size

CAKE's locality is not free at scale: it falls 1.0 → 0.55 as N goes 1 → 400. Cause: the firing gate is
...
Even so, at every N tested CAKE's *combined* behaviour dominates ROME's: CAKE at N=400 is
...
Sequential editing is where CAKE's design pays off most clearly: **near-perfect retention of hundreds
of edits at ~1600× lower per-edit write cost than ROME (CAKE 4.6 ms/edit vs ROME 7.6 s/edit), while
...
## Files
- `bench_sequential.py` (CAKE/base/RAG/finetune), `bench_sequential_edit.py` (ROME/MEMIT sequential)
