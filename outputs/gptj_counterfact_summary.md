# INLAY at the 6B tier — GPT-J-6B CounterFact

The GPT-2-XL results left one central question for the "state of the art" claim: does INLAY's win hold
on a standard, larger model, or was it an artifact of GPT-2-XL? This ports INLAY to **GPT-J-6B** — the
...
The semantic-key redesign made INLAY model-agnostic. The addressing key is a MiniLM sentence embedding,
not a hidden state, so the only model surfaces INLAY touches are `config.hidden_size` and
...
per-model layer selection and (for MEMIT) per-model covariance statistics; INLAY needs a scalar rescale.
...
| **INLAY v3** | **1.00** | **0.85** | **0.828** | **0.886** | ~0 | **0** |

INLAY gate 0.45 (selected on a held-out tune split); ROME layer 5, 20 grad steps, no covariance.

**Among methods that actually edit, INLAY leads every axis and the combined score (0.886 vs ROME 0.751)
...
| model | INLAY ES | PS | NS | INLAY score | ROME score |
|---|---|---|---|---|---|
| GPT-2-XL (1.5B) | 1.00 | 0.86 | 0.859 | **0.902** | 0.665 |
| GPT-J-6B (6B) | 1.00 | 0.85 | 0.828 | **0.886** | 0.751 |

INLAY's score is essentially flat across a 4× model-size jump (0.902 → 0.886). ROME actually improves at
6B (0.665 → 0.751) — GPT-J's cleaner MLP structure suits ROME — but still trails INLAY by ~0.13. The
GPT-2-XL result was not an artifact; **the method transfers.**

## What this does and does not settle for "state of the art"

**Settles:** INLAY's advantage is not model-specific. On two standard models, single-edit CounterFact,
against ROME/RAG/base with an honest held-out protocol, INLAY wins — gradient-free, non-destructive,
...
- **Competitor set.** ROME is beaten, but the current *sequential/lifelong* editors — WISE, GRACE,
  AlphaEdit — are INLAY's real conceptual rivals and are still unrun. EasyEdit ships GPT-J-6B hparams for
  WISE and GRACE, so this is the natural next step.
- **Portability / ripple.** INLAY plays back stored answer tokens; it retrieves rather than re-derives.
...
## Files
- `eval_cf_gptj.py` (INLAY, held-out gate, fp16), `eval_cf_edit_gptj.py` (ROME native metrics),
