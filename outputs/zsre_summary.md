# zsRE benchmark — INLAY v2 on the second standard knowledge-editing test
...
| **INLAY v2 (semantic key)** | 0.53 | 0.52 | **1.00** | 0.624 | 0.3 s |

*INLAY v2 reported held-out: gate 0.3 selected on a 50-record tune split, metrics on a disjoint
...
- **PS (0.52) ≈ ES (0.53)** — paraphrases address the written slot just as reliably as the original
  ...
  constructed benchmark. (For reference, INLAY v1's raw-hidden-state key gave PS=0.15 on CounterFact.)
...
## Why INLAY's overall score (0.62) trails ROME (0.91) here

Not an addressing failure — a **value-representation** limit. INLAY's stored value is the unembedding
...
## Cross-benchmark summary (INLAY v2)
...
is high on both**. Where INLAY trails (zsRE), the cause is isolated and identified: the single-token
value, not the addressing. That's the next thing to fix.

## Files
- `eval_zsre.py` (base/RAG/INLAY/finetune), `eval_zsre_edit.py` (ROME/MEMIT via EasyEdit)
- `zsre.json` / `zsre.csv` — all numbers incl. INLAY gate-sweep tune curve
