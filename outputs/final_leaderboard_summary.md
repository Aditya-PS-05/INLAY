# Final GPT-J-6B leaderboard — AlphaEdit added, INLAY scaled to N=1000

Two closing tests for the SOTA question: (1) add **AlphaEdit**, the last major weight-editor, and
(2) **scale N from 100 to 1000** to confirm INLAY's lead is not a small-sample artifact. Both are done.

## Full leaderboard (GPT-J-6B, CounterFact single-edit)

| method | N | ES | PS | NS | **score** | grad |
|---|---|---|---|---|---|---|
| **INLAY v3** | 1000 | 1.00 | 0.868 | 0.835 | **0.896** | 0 |
...
Gradient-free methods (Base/RAG/INLAY) at N=1000; weight/adapter editors at N=100 (~13 s/edit — N=1000
would be 3–4 h each, and their N=100 numbers are the fair comparison point). INLAY gate selected on a
held-out tune split.

**Ranking: INLAY 0.896 > ROME 0.751 > WISE 0.653 > RAG 0.546 > AlphaEdit 0.426 > GRACE/MEMIT/Base.**

## The two new results

**1. INLAY is stable at 10× the sample.**
...
single-fact efficacy. Score 0.426, below ROME and INLAY. (Setup note: AlphaEdit needed a one-line
...
## So — is INLAY state of the art?

Here is the honest, complete accounting after all the work:

**What is now established:**
- INLAY leads **every method measured** — base, RAG, fine-tune, ROME, MEMIT, WISE, GRACE, AlphaEdit — on
...
- INLAY holds retention flat where ROME collapses in the **sequential** stress test, at ~1600× lower
  per-edit write cost, gradient-free.

**The one honest caveat that remains:**
- INLAY **over-fires** on same-subject/different-relation queries (0.97 without the relation gate, 0.67
...
built on, INLAY is **the strongest method in this comparison — a defensible "state of the art on edit
...
LLM probes) — and that axis is exactly where INLAY is expected to concede to weight-editors, so it would
