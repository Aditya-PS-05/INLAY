...
| **CAKE (this)** | **1.00** | 0.25 | **1.00** | **0.5 s** | **0** |

## What this benchmark shows — the honest reading

**CAKE holds up on its core promise even on the competitors' home turf.** All 24 facts recalled
...
2. **This is not the CounterFact leaderboard.** Published MEMIT reports high scores editing
   ...
   **fabricated facts, strict substring-match greedy decoding, and the 1.5B GPT-2-XL** — a harder,
   ...
   not be read as "CAKE beats MEMIT in general." They show how the methods behave *on this specific,
   controlled setup*.
...
## CAKE's real, unchanged weakness: generalization (0.25)

CAKE recalls facts perfectly on the exact prompt but only 1-in-4 paraphrases. This is the honest
limitation, and it's structural: CAKE keys on the raw hidden state of the *written* phrasing, so a
...
slot — the clear next step if we push CAKE further.

## The fair conclusion

On this controlled multi-subject benchmark, CAKE gives the best **combined** profile (efficacy +
...
## Reproducibility
`bench_multi.py <model> <cake_layer> <cake_alpha> <n_sub>` runs base/RAG/fine-tune/CAKE;
