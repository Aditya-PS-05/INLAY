# CAKE vs the sequential/lifelong editors, and the portability weakness

Two questions remained for the "state of the art" claim after the GPT-J-6B port: (step 2) how does
CAKE compare to the *sequential/lifelong* editors WISE and GRACE — its real conceptual rivals — and
(step 3) does CAKE have a genuine weakness on the one axis where a retrieval method should be weak,
**portability** (does an edit propagate to related facts rather than being blindly played back)?

Both were run on GPT-J-6B. The answer to step 2 is that CAKE still leads. The answer to step 3 is
**yes, CAKE has a real weakness** — it over-fires on same-subject/different-relation queries — and a
...
| **CAKE v3** | **1.00** | **0.85** | **0.828** | **0.886** | ~0 |

**Ranking: CAKE 0.886 > ROME 0.751 > WISE 0.653 > RAG 0.515 > Base/GRACE.**

- **WISE** has the profile its design predicts: perfect efficacy and perfect locality (its side-memory
  is non-destructive, like CAKE) but weak paraphrase generalization (PS 0.385) — it stores the exact
  edit and doesn't generalize to reworded questions. CAKE's semantic key gives it much better PS (0.85).
...
CAKE beats the sequential-editing rivals on this benchmark. But WISE/GRACE are built for *long streams*
of edits — the fair comparison there is the sequential stress test already in this project (where CAKE
...
| Base | 0.03 | 0.00 |
| RAG | 0.63 | 0.00 |
| **CAKE (rel_gate 0)** | **1.00** | **0.97** |

CAKE propagates perfectly but **over-fires 97% of the time** — ask "Singled Out was hosted by ___" after
editing its debut network to CBS, and CAKE plays back "CBS". RAG doesn't (the model still reasons about
which attribute is asked). **This is the structural signature of retrieval-not-rederivation** and is
CAKE's genuine remaining weakness — exactly where a stored-answer method is expected to be weak.

**Root cause (measured).** CAKE's gate keys on a MiniLM embedding of the whole prompt, which is dominated
...
- CAKE leads every method measured on single-edit CounterFact at both model sizes, **including the
  sequential-editing rivals WISE and GRACE.**
- CAKE has a **real, characterized weakness**: it over-fires on same-subject/different-relation queries
...
- The remaining honest gates: full-benchmark N (vs 100), a native RippleEdits/portability dataset (vs
  LLM-generated probes), and AlphaEdit. CAKE is the strongest method *on the axes measured here*, with a
  named weakness and a partial fix — not an unqualified SOTA claim.

## Files
- `eval_cf_wise_grace.py` (WISE/GRACE), `eval_comp_portability.py` (compositional probe, cake/base/RAG),
